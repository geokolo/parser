...
use function Caleido\Parser\data_get;
use function Caleido\Parser\alias_get;
use function Caleido\Parser\object_get;
...
class Document extends BaseDocument
{
    ...
    {
        if (\preg_match('/^(.*)\[(.*)\]$/', $use, $matches) && $content instanceof SimpleXmlIterator) {
            return $this->getValueCollection($content, $matches, $default);
        } elseif (\strpos($use, '::') !== false && $content instanceof SimpleXmlIterator) {
    ...
    /**
     * Cast value to string only when it is an instance of SimpleXMLIterator.
     *
     * @param  mixed  $value
     *
     * @return mixed
     ...
     * @return mixed
     */
    protected function getValueAttribute(SimpleXMLIterator $content, string $use, $default = null)
    {
     ...
     * @return mixed
     */
    protected function getRawValueAttribute(SimpleXMLIterator $content, ?string $use, $default = null)
    {
     ...
     * @return mixed
     */
    protected function getValueData(SimpleXMLIterator $content, ?string $use, $default = null)
    {
     ...
     * @return mixed
     */
    protected function getValueCollection(SimpleXMLIterator $content, array $matches, $default = null)
    {
        $parent = $matches[1];
        $namespace = null;
        ...
        if (! $collection instanceof SimpleXMLIterator) {
            return $default;
        }
     ...
     * @return array
     */
    protected function parseValueCollection(SimpleXMLIterator $content, array $uses): array
    {
        $value = [];
        $result = [];
