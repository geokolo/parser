...
abstract class Document
{
    ...
    public function parse(array $schema, array $config = []): array
    {
        ...
        for( $this->originalContent->rewind(); $this->originalContent->valid(); $this->originalContent->next() ) {
           
            $interim = [];
            $this->content = $this->originalContent->current();
            ...
            foreach ($schema as $key => $data) {
                ...
                if(\Str::startsWith($values, $name)){
                    $values = \Str::replaceFirst($name, '', $values);
                }
                if(\Str::startsWith($values, '.')){
                    $values = \Str::replaceFirst('.', '', $values);
                }

                $value = $this->parseData([$keys => $values]);
